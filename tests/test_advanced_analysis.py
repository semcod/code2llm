...
"""
    ...
    assert (
        simple.complexity.get(
            "cyclomatic_complexity", simple.complexity.get("cyclomatic", 0)
        )
        == 1
    )
    assert (
        complex_f.complexity.get(
            "cyclomatic_complexity", complex_f.complexity.get("cyclomatic", 0)
        )
        > 1
    )
    assert complex_f.complexity.get(
        "cyclomatic_complexity", complex_f.complexity.get("cyclomatic", 0)
    ) > simple.complexity.get(
        "cyclomatic_complexity", simple.complexity.get("cyclomatic", 0)
    )
...
def test_graph_metrics():
    ...
    for name in func_names:
        result.functions[name] = FunctionInfo(
            name=name, qualified_name=name, file="test.py", line=1
        )
...
def test_circular_dependency():
    ...
    # A -> B -> A
    result.functions["A"] = FunctionInfo(
        name="A", qualified_name="A", file="test.py", line=1
    )
    result.functions["B"] = FunctionInfo(
        name="B", qualified_name="B", file="test.py", line=1
    )
