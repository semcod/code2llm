...
"""
    ...
    result.functions = {
        ...
        "test.b": FunctionInfo(
            name="b", qualified_name="test.b", file="test.py", line=5, calls=[]
        ),
        "test.c": FunctionInfo(
            name="c", qualified_name="test.c", file="test.py", line=7, calls=["test.b"]
        ),
    }
...
def test_smell_detection():
    ...
    result = AnalysisResult(project_path=".", analysis_mode="static")
        ...
        name="god_func", qualified_name="test.god_func", file="test.py", line=10
    )
    fi.complexity = {"cyclomatic_complexity": 20}
    result.functions = {"test.god_func": fi}
    result.metrics = {"test.god_func": {"fan_out": 15, "fan_in": 1}}
