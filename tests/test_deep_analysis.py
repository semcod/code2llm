...
""")
    ...
    try:
        ...
        print(
            f"DEBUG: Found files: {[os.path.basename(f) for f, _ in analyzer._collect_files(tmp_path)]}"
        )
        ...
        assert func_a_name is not None, (
            f"caller_func not found. Available: {list(result.functions.keys())}"
        )
