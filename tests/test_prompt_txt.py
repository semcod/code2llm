...
class TestPromptTxtGeneration:
    ...
    def test_prompt_txt_not_generated_without_code2logic_format(
        self, temp_output_dir, mock_args
    ):
        ...
        assert not prompt_file.exists(), (
            "prompt.txt should not be generated without code2logic format"
        )

    def test_prompt_txt_generated_with_code2logic_format(
        self, temp_output_dir, mock_args
    ):
        ...
        assert prompt_file.exists(), (
            "prompt.txt should be generated with code2logic format"
        )
    ...
    def test_prompt_txt_lists_existing_files(self, temp_output_dir, mock_args):
        ...
        assert "map.toon.yaml" in content, (
            "map.toon.yaml should be listed when it is missing"
        )
    ...
    def test_prompt_txt_shows_missing_files(self, temp_output_dir, mock_args):
        ...
        assert "Missing files" in content, (
            "Missing section should be present when files don't exist"
        )
    ...
    def test_prompt_txt_contains_task_instructions(self, temp_output_dir, mock_args):
        ...
        assert "You are an AI assistant" in content, (
            "Main instruction should be present"
        )
        assert "Task:" in content, "Task section should be present"
        assert "refactoring brief" in content, (
            "Task section should instruct refactoring"
        )
    ...
    def test_prompt_txt_prioritizes_blockers_when_validation_and_duplication_exist(
        self, temp_output_dir, mock_args
    ):
        ...
        (temp_output_dir / "project" / "duplication.toon.yaml").write_text(
            "duplication"
        )
        ...
        assert "Priority Order:" in content, "Priority section should be present"
        assert "P0 — Fix validation issues" in content, (
            "Validation issues should be first priority"
        )
        assert "P0/P1 — Remove duplicated logic" in content, (
            "Duplication issues should be prioritized next"
        )
        assert "P1 — Split or simplify the highest-CC / god modules" in content, (
            "Analysis-driven refactoring should follow blockers"
        )
        assert "P2 — Keep the compact project overview" in content, (
            "Project overview should be preserved as a later priority"
        )
...
class TestCode2logicExport:
    ...
    def test_export_code2logic_adds_quiet_flag_when_not_verbose(self, temp_output_dir):
        ...
        with patch(
            "code2llm.cli_exports.code2logic.shutil.which",
            return_value="/usr/bin/code2logic",
        ), patch(
            "code2llm.cli_exports.code2logic.subprocess.run", return_value=completed
        ) as run_mock, patch(
            "code2llm.core.toon_size_manager.manage_toon_size",
            return_value=[temp_output_dir / "project.toon"],
    ...
    def test_export_code2logic_does_not_add_quiet_flag_when_verbose(
