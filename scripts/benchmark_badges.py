...
def generate_badges(metrics: Dict[str, str]) -> List[Dict[str, str]]:
    ...
    if "cc_avg" in metrics:
        badges.append({"label": "CC̄", "value": metrics["cc_avg"], "color": "blue"})
    if "max_cc" in metrics:
        badges.append(
            {"label": "max-CC", "value": metrics["max_cc"], "color": "orange"}
        )
    if "god_modules" in metrics:
        badges.append(
            {"label": "god-modules", "value": metrics["god_modules"], "color": "red"}
        )
    if "high_cc" in metrics:
        badges.append(
            {"label": "high-CC", "value": metrics["high_cc"], "color": "yellow"}
        )
    if "hub_types" in metrics:
        badges.append(
            {"label": "hub-types", "value": metrics["hub_types"], "color": "green"}
        )
    if "total_funcs" in metrics:
        badges.append(
            {"label": "functions", "value": metrics["total_funcs"], "color": "purple"}
        )

    return badges


def generate_format_quality_badges(
    format_scores: Dict[str, Dict],
) -> List[Dict[str, str]]:
    ...
    if format_scores:
        ...
        for i, (format_name, scores) in enumerate(sorted_formats):
            ...
            badges.append({"label": f"#{rank}", "value": format_name, "color": color})

        # Add total score
        total_score = (
            sum(s.get("total_score", 0) for s in format_scores.values())
            / len(format_scores)
            if format_scores
            else 0
        )
        badges.append(
            {"label": "Avg Score", "value": f"{total_score:.1f}/100", "color": "blue"}
        )
...
def create_html(
    badges: List[List[Dict[str, str]]], title: str = "Code2LLM Benchmarks"
) -> str:
        ...
        if category_badges:
            ...
            for badge in category_badges:
                shield_url = get_shield_url(
                    badge["label"], badge["value"], badge["color"]
                )
                html += f"""
                <tr>
                    <td>
