...
def demo_standard_strategy():
    ...
    for update in analyzer.analyze_streaming("sample_project"):
        if update["type"] == "file_complete":
            ...
            print(
                f"   Total: {file_count} files, {function_count} functions, {class_count} classes"
            )
...
def demo_deep_strategy():
    ...
    for update in analyzer.analyze_streaming("sample_project"):
        if update["type"] == "file_complete":
            ...
            print(
                f"🔍 Deep analysis: {Path(update['file']).name} "
...
def demo_memory_limited():
    ...
    for update in analyzer.analyze_streaming("sample_project"):
        if update["type"] == "file_complete":
            ...
            print(
                f"✅ Memory-limited analysis completed in {update['elapsed_seconds']:.2f}s"
            )
            break
...
def demo_custom_progress():
    ...
    class ProgressTracker:
        ...
        def __call__(self, update):
            ...
            if phase == "quick_scan":
                ...
                print(
                    f"\r🔄 Scanning: {current}/{total} files "
                    f"({rate:.1f} files/s, ETA: {eta:.1f}s)",
                    end="",
                    flush=True,
                )
    ...
    for update in analyzer.analyze_streaming("sample_project"):
        if update["type"] == "complete":
            ...
            print(f"\n✅ Completed in {elapsed:.2f}s ({tracker.files_processed} files)")
            break
