...
class DataAnalyzer:
    ...
    def _find_state_patterns(self, result: AnalysisResult) -> list:
        ...
        for func_name, func in result.functions.items():
            name_lower = func.name.lower()
            if any(
                ind in name_lower for ind in state_indicators + transition_indicators
            ):
                affected_states = []
                for call in list(func.calls)[:10]:
                    call_func = result.functions.get(call)
                    if call_func and any(
                        ind in call_func.name.lower() for ind in state_indicators
                    ):
                ...
                patterns.append(
                    {
                        ...
                        "description": func.docstring[:150]
                        if func.docstring
                        else "N/A",
                    }
                )
            ...
            for called in list(func.calls)[:15]:
                ...
                if func_module != called_module and called in result.functions:
                    ...
                    module_data_flow[key]["data_functions"].append(
                        {"caller": func_name, "callee": called}
                    )
                    module_data_flow[key]["call_count"] += 1
        deps = sorted(
            module_data_flow.values(), key=lambda x: x["call_count"], reverse=True
        )
        for dep in deps:
            ...
            if any(ind in name_lower for ind in event_indicators) or any(
                ...
                for called in list(func.calls)[:10]:
                    ...
                    if called_func and any(
                        ind in called_func.name.lower()
                        for ind in event_indicators + ["handle", "process"]
                    ):
                ...
                flows.append(
                    {
                        ...
                        "type": "emitter"
                        if "emit" in name_lower or "trigger" in name_lower
                        else "handler",
                        "handlers": handlers[:5],
                        "description": func.docstring[:150]
                        if func.docstring
    ...
    def _detect_types_from_name(self, func_name: str, doc: str) -> list:
        ...
        return [
            ...
            if any(ind in name_lower or ind in doc for ind in inds)
        ]

    def _create_type_entry(
        self, type_key: str, detected: list, params: list, returns: list
    ) -> dict:
    ...
    def _update_type_stats(
        self, entry: dict, func_name: str, func_module: str, calls: list
    ) -> None:
        ...
        for func_name, func in result.functions.items():
            ...
            if detected or params or returns:
                ...
                if type_key not in data_types:
                    data_types[type_key] = self._create_type_entry(
                        type_key, detected, params, returns
                    )
                func_module = (
                    func_name.rsplit(".", 1)[0] if "." in func_name else "root"
                )
                self._update_type_stats(
                    data_types[type_key], func_name, func_module, func.calls
                )
    ...
    def _identify_process_patterns(self, result: AnalysisResult) -> list:
            ...
            for p_type, inds in indicators.items():
                if any(ind in name or ind in doc for ind in inds):
                    patterns[p_type].append(
                        {
                            "function": func_name,
                            "description": func.docstring[:100]
                            if func.docstring
                            else "N/A",
        ...
        for p_type, funcs in patterns.items():
            res.append(
                {"pattern_type": p_type, "functions": funcs[:10], "count": len(funcs)}
            )
...
class OptimizationAdvisor:
    ...
    def analyze_optimization_opportunities(
        ...
        """Analyze optimization opportunities in data handling."""
        return DataAnalyzer()._analyze_optimization_opportunities(
            result, data_types, dfg
        )
