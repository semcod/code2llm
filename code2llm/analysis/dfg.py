...
class DFGExtractor(ast.NodeVisitor):
    ...
    def __init__(self, config: Config):
        ...
        self.variable_uses: Dict[str, List[int]] = defaultdict(
            list
        )  # variable -> nodes where used
        self.current_scope = ""
        self.scope_stack = []
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        self.current_scope = (
            self.scope_stack[-1] if self.scope_stack else self.module_name
        )
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        for target in targets:
            ...
            if scoped_name not in self.result.data_flows:
                self.result.data_flows[scoped_name] = DataFlow(
                    variable=target, dependencies=set()
                )
        ...
        for dep in dependencies:
            scoped_dep = (
                f"{self.current_scope}.{dep}" if dep not in self.variable_defs else dep
            )
    ...
    def visit_For(self, node: ast.For):
        ...
        if isinstance(node.target, ast.Name):
            ...
            if scoped_name not in self.result.data_flows:
                self.result.data_flows[scoped_name] = DataFlow(
                    variable=loop_var, dependencies=set(iter_deps)
                )
            else:
            ...
            if deps:
                ...
                if flow_key not in self.result.data_flows:
                    self.result.data_flows[flow_key] = DataFlow(
                        variable=flow_key, dependencies=deps
                    )
                else:
