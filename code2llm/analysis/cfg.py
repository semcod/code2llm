...
class CFGExtractor(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        exit_node = self.new_node(
            "RETURN", f"RETURN:{func_name}", line=node.end_lineno or node.lineno
        )
    ...
    def visit_For(self, node: ast.For):
        ...
        loop_header = self.new_node(
            "FOR", f"for {target_str} in {iter_str}", line=node.lineno
        )
    ...
    def visit_Try(self, node: ast.Try):
        ...
        for handler in node.handlers:
            handler_node = self.new_node(
                "EXCEPT", self._format_except(handler), line=handler.lineno
            )
