...
class HierarchicalRepoSplitter:
    ...
    def _split_hierarchically(self, project_path: Path) -> List[SubProject]:
        ...
        for dir_path, files, estimated_kb in large_l1_dirs:
            level2_chunks = self._split_level2_consolidated(
                dir_path, project_path, target_per_chunk
            )
    ...
    def _merge_small_l1_dirs(
        self, small_l1_dirs: List[Tuple[Path, List, int]], project_path: Path
    ) -> List[SubProject]:
        ...
        # Sort by priority (highest first)
        sorted_dirs = sorted(
            small_l1_dirs, key=lambda x: calculate_priority(x[0].name, 1), reverse=True
        )
        ...
        for dir_path, files, estimated_kb in sorted_dirs:
            ...
            if current_chunk_files and (current_size + estimated_kb > effective_limit):
                # Flush current chunk
                chunk_name = (
                    "_".join(current_chunk_names)
                    if len(current_chunk_names) <= 3
                    else f"batch_{len(chunks) + 1}"
                )
    ...
    def _split_level2_consolidated(
        self, level1_path: Path, project_path: Path, target_chunk_kb: int
    ) -> List[SubProject]:
        ...
        for file_path, module_name in all_files:
            ...
            if current_files and (current_size + file_kb > effective_limit):
                chunk_name = (
                    f"{level1_path.name}_part{chunk_num}"
                    if chunk_num > 1
                    else level1_path.name
                )
                chunks.append(
                    SubProject(
                        name=chunk_name,
    ...
    def _process_large_dirs(
        ...
        for dir_path, files, estimated_kb, priority in large_dirs:
            file_chunks = self._chunk_by_files(
                files, level1_path.name, dir_path.name, dir_path, project_path
            )
            chunks.extend(file_chunks)
        return chunks

    def _process_level1_files(
        self, level1_path: Path, project_path: Path
    ) -> List[SubProject]:
    ...
    def _merge_small_dirs(
        ...
        for dir_path, files, estimated_kb, priority in small_dirs:
            ...
            if current_chunk_files and (
                current_size + estimated_kb > self.size_limit_kb
            ):
                # Flush current chunk
