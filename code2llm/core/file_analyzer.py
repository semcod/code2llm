...
class FileAnalyzer:
    ...
    def _route_to_language_analyzer(
        self, content: str, file_path: str, module_name: str, ext: str
    ) -> Dict:
        """Dispatch file content to the correct language analyzer."""
        if ext == ".py":
            return self._analyze_python(content, file_path, module_name)
        if ext in (".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs"):
            return analyze_typescript_js(
                content, file_path, module_name, ext, self.stats
            )
        if ext == ".go":
    ...
    def _analyze_ast(
        self, tree: ast.AST, file_path: str, module_name: str, content: str
    ) -> Dict:
    ...
    def _process_class(
        self, node: ast.ClassDef, file_path: str, module_name: str, result: Dict
    ) -> None:
    ...
    def _process_cfg_block(
        ...
        for stmt in body:
            if isinstance(stmt, ast.If):
                current = self._process_if_stmt(
                    stmt, current, exit, func_name, func_info, result, depth, max_depth
                )
            elif isinstance(stmt, (ast.For, ast.While)):
                current = self._process_loop_stmt(
                    stmt, current, func_name, func_info, result, depth, max_depth
                )
            elif isinstance(stmt, ast.Return):
                return self._process_return_stmt(
                    stmt, current, exit, func_name, func_info, result
                )
    ...
    def _process_if_stmt(
        ...
        result["nodes"][node_id] = FlowNode(
            id=node_id, type="IF", label="if", function=func_name, line=stmt.lineno
        )
