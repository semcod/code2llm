...
def _export_mermaid(args, result, output_dir: Path):
    ...
    if getattr(args, "flow_detail", False):
        exporter.export_flow_detailed(
            result, str(output_dir / "flow_detailed.mmd"), include_examples
        )
        mmd_files.append("flow_detailed.mmd")
    if getattr(args, "flow_full", False):
        exporter.export_flow_full(
            result, str(output_dir / "flow_full.mmd"), include_examples
        )
    ...
    if args.verbose:
        ...
        print(
            f"  - Mermaid: {output_dir}/*.mmd ({', '.join(files)}) ({elapsed_mmd:.2f}s)"
        )
...
def _export_data_structures(args, result, output_dir: Path):
    ...
    yaml_exporter = YAMLExporter()
    yaml_exporter.export_data_structures(
        result, str(output_dir / "data_structures.yaml"), compact=True
    )
    if args.verbose:
