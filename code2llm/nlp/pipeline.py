...
class NLPPipeline:
    ...
    def _step_resolve_entities(
        ...
        return self.entity_resolver.resolve(
            normalized_query, context=context, expected_types=expected_types
        )

    def _infer_entity_types(
        self, intent_result: IntentMatchingResult
    ) -> Optional[List[str]]:
    ...
    def _calculate_entity_confidence(
        self, entity_result: EntityResolutionResult
    ) -> float:
    ...
    def _apply_fallback(self, result: NLPPipelineResult) -> NLPPipelineResult:
        ...
        if result.intent_result.get_confidence() < 0.3:
            ...
            self.intent_matcher.config.fuzzy_threshold = (
                self.config.intent_matching.fuzzy_threshold
            )
