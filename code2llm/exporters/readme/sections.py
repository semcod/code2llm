...
def build_core_files_section(
    existing: Dict[str, bool], insights: Dict[str, Any]
) -> str:
...
def build_viz_files_section(existing: Dict[str, bool]) -> str:
    ...
    has_mermaid = (
        existing.get("flow.mmd")
        or existing.get("calls.mmd")
        or existing.get("compact_flow.mmd")
    )
    if not has_mermaid:
        return ""
