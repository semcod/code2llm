...
@export_format(
    "context", description="Context markdown for LLM consumption", extension=".md"
)
class ContextExporter(BaseExporter):
    ...
    def _get_architecture_by_module(self, result: AnalysisResult) -> List[str]:
        ...
        for mod_name, mod in result.modules.items():
            if len(mod.functions) > 0 or len(mod.classes) > 0:
                module_stats.append(
                    (mod_name, len(mod.functions), len(mod.classes), mod.file)
                )
        ...
        for mod_name, f_count, c_count, file_path in module_stats[:20]:
            ...
            if c_count > 0:
                lines.append(f"- **Classes**: {c_count}")
            if file_path:
    ...
    def _get_key_classes(self, result: AnalysisResult) -> List[str]:
        ...
        lines = ["## Key Classes", ""]
        class_list = sorted(
            result.classes.items(), key=lambda x: len(x[1].methods), reverse=True
        )
        for cls_name, cls in class_list[:20]:
            ...
            if cls.methods:
                lines.append(f"- **Key Methods**: {', '.join(cls.methods[:10])}")
            if cls.bases:
    ...
    def _get_data_transformations(self, result: AnalysisResult) -> List[str]:
        ...
        data_funcs = [
            (name, f)
            for name, f in result.functions.items()
    ...
    def _get_api_surface(self, result: AnalysisResult) -> List[str]:
        ...
        public_funcs = sorted(
            [
                ...
                if not f.name.startswith("_") and "." in n
            ],
    ...
    def _format_sub_flow(
        self, sub_flow: str, called: str, is_cross_module: bool
    ) -> list:
    ...
    def _trace_flow(
        ...
        calls_by_module, func_module = self._group_calls_by_module(
            func.calls, func_name
        )

        shown = 0
        for mod, calls in sorted(
            calls_by_module.items(), key=lambda x: x[0] != func_module
        ):
            for called in calls[:2]:
                ...
                if called_func and called not in visited:
                    sub_flow = self._trace_flow(
                        called, called_func, result, depth - 1, visited.copy()
                    )
                    lines.extend(
                        self._format_sub_flow(sub_flow, called, mod != func_module)
                    )
                    shown += 1
        return "\n".join(lines)
