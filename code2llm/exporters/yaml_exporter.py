...
class YAMLExporter(BaseExporter):
    ...
    def export_separated(
        self, result: AnalysisResult, output_dir: str, compact: bool = True
    ) -> None:
        ...
        with open(output_path / "orphans.yaml", "w", encoding="utf-8") as f:
            yaml.dump(
                {"functions": orphans}, f, default_flow_style=False, allow_unicode=True
            )

    def export_split(
        self, result: AnalysisResult, output_dir: str, include_defaults: bool = False
    ) -> None:
    ...
    def _collect_edges(
        self, result: AnalysisResult, max_calls: int, max_edges: int
    ...
    def _should_add_edge(
        func_name: str, resolved: Optional[str], seen_pairs: Set[Tuple[str, str]]
    ) -> bool:
    ...
    def _build_nodes(
        self, result: AnalysisResult, connected: Set[str]
    ) -> Dict[str, Dict]:
    ...
    def _group_by_module(
        result: AnalysisResult, connected: Set[str]
    ) -> Dict[str, List[str]]:
        ...
        for module, funcs in sorted(modules.items()):
            ...
            for func in funcs[:10]:  # Show up to 10 funcs per module
                if func in nodes:
                    data = nodes[func]
                    lines.append(
                        f"    {data['name']}  CC={data['cyclomatic_complexity']}  out:{data['calls_out']}"
                    )
        return lines
