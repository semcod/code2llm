...
def render_header(
    ...
    stats_line = _render_stats_line(
        included_funcs, included_files, total_lines, lang_str
    )
    alerts_line = _render_alerts_line(included_funcs)
    hotspots_line = _render_hotspots_line(included_funcs)
    trend = load_evolution_trend(
        P(output_path).with_name("evolution.toon.yaml"), stats_line.get("avg_cc", 0.0)
    )
...
def _render_stats_line(
    funcs: List[FunctionInfo], files: List[ModuleInfo], total_lines: int, lang_str: str
) -> Dict[str, Any]:
