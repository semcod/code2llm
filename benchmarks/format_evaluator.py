...
def _detect_problems(content: str) -> Dict[str, bool]:
    ...
    return {
        "god_function": bool(
            re.search(r"god|!! split|CC[=≥]\s*(1[5-9]|[2-9]\d)", content)
        ),
        "hub_type": bool(
            re.search(r"hub.?type|consumed.?\d|HUB|fan.?in.?\d", content, re.I)
        ),
        "duplicate_class": bool(
            re.search(r"DUP|duplicate|×DUP|identical", content, re.I)
        ),
        "impure_pipeline": bool(
            re.search(r"impure|IO|side.?effect.*pipeline|purity.*0", content, re.I)
        ),
        "dead_code": bool(
            re.search(r"dead.?code|unused|unreachable|never.?called", content, re.I)
        ),
        "high_fan_out": bool(
            re.search(r"fan.?out|fan=\d|hotspot|fan-out", content, re.I)
        ),
        "missing_types": bool(
            re.search(r"missing.?type|no.?annotation|untyped", content, re.I)
        ),
        "side_effect": bool(
            re.search(
                r"side.?effect|mutation|mutate|global|cache_result", content, re.I
            )
        ),
    }
...
def _detect_hub_types(content: str) -> Dict[str, bool]:
    ...
    for type_name in KNOWN_HUB_TYPES:
        type_pattern = (
            rf"{type_name}.*consumed|{type_name}.*fan|{type_name}.*←\d|{type_name}.*hub"
        )
...
def _check_structural_features(content: str) -> Dict[str, bool]:
    ...
    return {
        "has_call_chains": bool(
            re.search(r"→|calls:|──>|call.?chain|call.?graph", content, re.I)
        ),
        "has_type_info": bool(
            re.search(r"->\s*(str|int|dict|list|Result|Config|None)", content)
        ),
        "has_coupling_info": bool(
            re.search(r"COUPLING|fan-in|fan-out|import.*graph|←in", content, re.I)
        ),
        "has_severity_markers": bool(re.search(r"!!|🔴|🟡|critical|HEALTH", content)),
        "has_refactor_steps": bool(
            re.search(r"REFACTOR|split|extract|merge.*class", content, re.I)
        ),
        "has_side_effects": bool(
            re.search(r"SIDE.?EFFECT|pure|impure|IO|mutation", content, re.I)
        ),
        "has_purity_info": bool(re.search(r"purity|pure.*\d|PURITY", content, re.I)),
        "has_contracts": bool(
            re.search(r"CONTRACT|IN:|OUT:|input.*→.*output|invariant", content, re.I)
        ),
    }


def evaluate_format(
    name: str, content: str, path: Optional[Path] = None
) -> FormatScore:
