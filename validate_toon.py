...
from code2llm.parsers.toon_parser import is_toon_file, load_toon
...
def compare_classes(yaml_data, toon_data):
    ...
    print("  - YAML: Direct AST analysis (more accurate for empty classes)")
    print("  - TOON: Inferred from function grouping (only classes with methods)")
...
def compare_modules(yaml_data, toon_data):
    ...
    print("  - YAML: Direct module imports and file-level analysis")
    print("  - TOON: Inferred from function locations (more granular)")
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        print("Usage:")
        print(
            "  python validate_toon.py <analysis.toon>                      # Validate TOON only"
        )
        print(
            "  python validate_toon.py <analysis.yaml> <analysis.toon>      # Compare both formats"
        )
        return 1
